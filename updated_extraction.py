...
class JobProcessor:
    ...
    def insert_skills_to_db(self, skills_list, job_id):
        ...
        for skill_name in skills_list:
            ...
            if existing_skill:
                ...
                skill_ids.append(cursor.fetchone()['id'])
    ...
    def insert_job_complete(self, job_data):
        ...
        try:
            ...
            skill_ids = self.insert_skills_to_db(skills_list, job_id)
    ...
    def process_all_jobs(self, json_file_path):
        """Process all jobs from JSON file and populate all three tables"""
        try:
            ...
            for i, job in enumerate(jobs, 1):
                if isinstance(job, dict):
                    if self.insert_job_complete(job):
                        processed_count += 1
                    else:
                        failed_count += 1
            ...
            self.db_postgres.logger.info(
                f"Completed processing: {processed_count} successful, {failed_count} failed out of {total_jobs} total"
            )
    ...
    def clear_and_restart(self):
        ...
        self.db_postgres.logger.info("Cleared all job, skillgroup, and skill data")
